...
export default function Header({ t, lang, setLang, onNavigate }) {
  ...
  return <header className="site-header">
    <nav className="nav wrap" aria-label={t.nav_home}>
      <button className="logo" type="button" aria-label={t.aria_back_top} onClick={() => navigate('hero')}>
      ...
      <button className="menu-toggle" type="button" aria-label={t.aria_open_menu} aria-expanded={open} onClick={() => setOpen(v=>!v)}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M3 12h18M3 18h18"/></svg></button>
    </nav>
